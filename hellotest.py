...
def populateHome(frame):
    ...
    def transaction1():
        ...
        def add_to_database():
        	try:
        		conn.execute("INSERT into Expense(username,item,tag,type,amount,edate) VALUES ('%s','%s','%s','%s','%d','%s');"%('Rounak',name.get(),tg.get(),typ.get(),int(amt.get()),today))
        		conn.commit()
    ...
    #<data retrival>
    cursor=conn.execute("SELECT amount,tag from Expense where type=\"expense\" and edate>='%s' and edate<='%s'"%(startdate,today))
    ...
    exp=[0,0,0,0]
    for i in range(4):
        cursor=conn.execute("SELECT type,amount from Expense where edate>='%s' and edate<='%s'"%(mon[i],endmon[i]))
        rows=cursor.fetchall()
        for row in rows:
